...
void setseg(uint8_t seg)
{
	...
	switch(seg)
	{
		...
		break;	
	    case 20: PORTD &= ~(SEG_A | SEG_F | SEG_E | SEG_G);          //print F
	    break;
...
void print_statusbyte(unsigned char _statusbyte0, unsigned char _statusbyte1, unsigned char _statusbyte2)
{
	...
	if ((_statusbyte1 & (1 << STATUS_BIT1_L_TRIP_STAGE1)) || (_statusbyte2 & (1 << STATUS_BIT2_L_TRIP_STAGE2))) // protections trip
	{
		selectseg(1);
		setseg(20);
		PORTD |= (SEG_DP);
		...
		if (_statusbyte1 & (1 << STATUS_BIT1_L_TRIP_STAGE1))
		{
	    	setseg(1);			//F1
		} else
		{
	...
	else
	{
		...
		if (_statusbyte2 & (1 << STATUS_BIT2_SQUEEZE)) setseg(5); // Print S - SQUEEZE
		else
		{
			if (_statusbyte1 & (1 << STATUS_BIT1_L_ON_U)) setseg(11); // print U
		    if (_statusbyte1 & (1 << STATUS_BIT1_L_ON_D)) setseg(12); // print d
		}
		PORTD |= (SEG_DP);	
		_delay_ms(UPDATE_DISPLAY);
		...
		if (_statusbyte2 & (1 << STATUS_BIT2_WATERMODE)) addseg(SEG_G);
	...
	if ((_statusbyte1 & (1 << STATUS_BIT1_R_TRIP_STAGE1)) || (_statusbyte2 & (1 << STATUS_BIT2_R_TRIP_STAGE2))) //protections trip
	{	
		selectseg(3);
		setseg(20);
		PORTD |= (SEG_DP);
	...
	} else
	{
		...
		{
			if (_statusbyte1 & (1 << STATUS_BIT1_R_ON_U)) setseg(11); // print U
			if (_statusbyte1 & (1 << STATUS_BIT1_R_ON_D)) setseg(12);  //print d
		}
		PORTD |= (SEG_DP);
		_delay_ms(UPDATE_DISPLAY);
		...
		if (_statusbyte2 & (1 << STATUS_BIT2_WATERMODE)) addseg(SEG_G); // add "-" symbol in WATER_MODE
